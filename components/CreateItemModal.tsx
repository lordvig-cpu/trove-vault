...
export default function CreateItemModal({
  ...
  // Synchronize parentId whenever the modal opens or initialParentId changes
  useEffect(() => {
    if (isOpen) {
  ...
  // Flatten the hierarchy to make a simple dropdown selection list
  const flattenItems = (items: ItemRecord[], prefix = ''): { id: number; label: string }[] => {
    let result: { id: number; label: string }[] = [];
    for (const item of items) {
      result.push({ id: item.id, label: `${prefix}${item.name} (ID: ${item.id})` });
      if (item.children && item.children.length > 0) {
        result = result.concat(flattenItems(item.children, `${prefix}— `));
      }
    }
    return result;
  };
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-lg overflow-hidden shadow-2xl">
        ...
        <div className="flex items-center justify-between p-5 border-b border-slate-800">
          <h2 className="text-base font-bold text-white">
            {parentId ? 'Add Sub-Item' : 'Create New Root Item'}
          </h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div className="space-y-1">
            ...
            <input
              type="text"
              required
              placeholder="e.g. Sub-item name..."
              value={name}
              onChange={(e) => setName(e.target.value)}
              className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-xs text-white placeholder-slate-500 focus:outline-none focus:border-indigo-500 transition"
            />
          </div>
        ...
        <select
            value={parentId ?? ''}
            disabled={Boolean(initialParentId)} // Disables dropdown when adding a sub-item
